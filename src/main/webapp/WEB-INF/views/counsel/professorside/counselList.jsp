				...
				<ul class="nav nav-pills ml-auto p-2">
					...
					<div class=" alert alert-light" role="alert"
						...
						<p>
							...
							<br> (2)&nbsp;<strong style="color:blue;">비대면</strong>상담 에는 상담에 직접 답변 할 수 있습니다<br>
							<br> (3)&nbsp;<strong style="color:blue;">대면</strong>상담 에는 상담기록서를 작성하여 제출 할 수 있습니다.<br>
							<br> (4)&nbsp;<strong>답변은 수정 할 수 있습니다.</strong>
						</p>
					</div>
			<div class="card-body">
				...
				<table
					...
					<tbody>
							...
							<tr class="text-center counselListTr" data-toggle="modal" data-target="#counselListTr">
								...
								<td><fmt:formatDate value="${counselList.cnslReg }" pattern="yy/MM/dd" /></td> 
								<td><fmt:formatDate value="${counselList.cnslDt }" pattern="yy/MM/dd" /></td> 
								<c:if test="${counselList.cnslRpl != null }"> <td style="color:blue;">상담완료</td></c:if> 
 								<c:if test="${counselList.cnslRpl == null }"><td style="color:black;">상담 전</td></c:if>
							</tr>
						</c:forEach> 
					</tbody>
				</table>
	...
	<div class="tab-pane" id="tab_2">
				<table
					...
					<tbody>
							...
							<tr class="text-center nonFaceCounselListTr" data-toggle="modal" data-target="#nonFaceCounselListTr">
							    ...
							    <td><fmt:formatDate value="${nonFaceCounselList.cnslReg }" pattern="yy/MM/dd" /></td> 
 							    <td><fmt:formatDate value="${nonFaceCounselList.cnslDt }" pattern="yy/MM/dd" /></td> 
 								
 								<!-- 답변.. 날짜 ? -->
 								<c:if test="${nonFaceCounselList.cnslDt == null and nonFaceCounselList.cnslType == '비대면'}"> 
								<td style="color:black;">답변하기</td></c:if> 
								<c:if test="${nonFaceCounselList.cnslDt != null and nonFaceCounselList.cnslType == '반려'}">
								<td style="color: red">반려</td></c:if>
								<c:if test="${nonFaceCounselList.cnslDt != null and nonFaceCounselList.cnslType == '비대면'}"> 
								<td style="color:blue;">답변완료</td></c:if>
						</tr>
 						</c:forEach>
					</tbody>
				</table>
		...
		<div class="htb" 
			...
			<div class="hce"
				...
				<div class="hcD" style="left: 0.50mm; top: 0.50mm;">
					<div class="hcI" style="top: 2.74mm;">
						<div class="hls ps13"
							style="line-height: 2.79mm; white-space: nowrap; left: 0mm; top: -0.18mm; height: 3.53mm; width: 86.74mm;">
			...
			<div class="hce"
				style="left: 0mm; top: 116.29mm; width: 158.01mm; height: 56.01mm;">
							<textarea class="form-control" rows="3" id="cnslRpl" style="height:100rem;border-left:1px solid black; border-right:1px solid black;"></textarea>
			</div>
			<div class="hce"
			...
			<div class="hce"
				style="left: 0mm; top: 185.81mm; width: 126.92mm; height: 24.36mm;text-align: -webkit-center;">
					<input type="text" id="cnslYn" class="col-6 mt-4 text-center form-control form-control-border-rounded-0" value="" style="color:black; border:none;" />
			</div>
			<div class="hce"
			...
			<div class="modal-footer justify-content-align">
				...
				<button type="button" id="counselAnswerBtn" class="btn btn-outline-primary">등록</button>
				<button type="button" id="counselRejectBtn" class="btn btn-outline-secondary">반려</button>
...
// function kstDateConvertor(kstDate){
// 	     var convertDate = new Date(kstDate.toString().replace(/(\w+ \w+ \d+) [0-9:]+ KST (\d+)/, "$1 $2"));
// 	     var convertDateSet = [convertDate.getFullYear(),
// 	    	 			('0'+(convertDate.getMonth()+1)).slice(-2),
// 	    	 			('0'+convertDate.getDate()).slice(-2)].join('-');
	     
// 	     var resultDate = new Date(convertDateSet);
// 	     alert(resultDate)
// 	     $("#cnslDt").text(resultDate);
